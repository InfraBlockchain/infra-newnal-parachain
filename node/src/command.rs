...
fn load_spec(id: &str) -> std::result::Result<Box<dyn ChainSpec>, String> {
	Ok(match id {
		"dev" | "urauth-infra-dev" => Box::new(chain_spec::development_config()),
		"urauth-infra-local" => Box::new(chain_spec::urauth_local_config()),
		"urauth-infra" => Box::new(chain_spec::urauth_config()),
		"" => {
			log::warn!("No ChainSpec.id specified, so using default one, based on rococo-parachain runtime");
			Box::new(chain_spec::urauth_config())
		},
...
impl SubstrateCli for Cli {
	fn impl_name() -> String {
		"InfraBlockchain Newnal Parachain".into()
	}
	...
	fn description() -> String {
		format!(
			"InfraBlockchain Newnal Parachain\n\nThe command-line arguments provided first will be \
...
macro_rules! construct_partials {
	($config:expr, |$partials:ident| $code:expr) => {
		match $config.chain_spec {
			_ => {
				let $partials = new_partial::<infra_newnal_parachain_runtime::RuntimeApi, _>(
					&$config,
...
macro_rules! construct_async_run {
	(|$components:ident, $cli:ident, $cmd:ident, $config:ident| $( $code:tt )* ) => {{
		...
		runner.async_run(|$config| {
			let $components = new_partial::<infra_newnal_parachain_runtime::RuntimeApi, _>(
				&$config,
				crate::service::aura_build_import_queue::<_, AuraId>,
			)?;
	...
	match &cli.subcommand {
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				infra_newnal_parachain_runtime::RuntimeApi,
					AuraId,
